...
export function validateEnv(specs: EnvVarSpec[], serviceName: string): Record<string, any> {
  const errors: string[] = [];
  ...
  for (const spec of specs) {
    ...
    if (value === undefined || value === '') {
      if (spec.required) {
        ...
        // Make sure the default value is set in process.env so libraries/Prisma can read it
